...
async function startPairing(phone, timer) {
    ...
    // Baileys registers pre-keys on first connection.update, so we must
    // wait for that event rather than using a fixed delay.
    const code = await new Promise((resolve, reject) => {
        ...
        async function onConnUpdate(update) {
            if (!codeRequested && update.connection === 'connecting') {
                codeRequested = true;
                sock.ev.off('connection.update', onConnUpdate);
                clearTimeout(timeout);
                await delay(1500); // brief pause after pre-keys upload
                try {
                    let c = await sock.requestPairingCode(phone);
                    c = c?.match(/.{1,4}/g)?.join('-') || c;
                    const bots = activeBots.get(phone);
                    if (bots) bots.code = c;
                    console.log(`📱 Pairing: +${phone} | Code: ${c}`);
                    resolve(c);
                } catch (err) {
                    try { sock.end(); } catch {}
                    reject(new Error('Could not generate pairing code. Make sure number is on WhatsApp.'));
                }
            }
        }
...
async function reconnectBot(phone) {
    try {
        ...
        const sock = makeWASocket({
            ...
            keepAliveIntervalMs: 25000, markOnlineOnConnect: false,
            getMessage: async (key) => {
                const msg = await userStore.loadMessage(jidNormalizedUser(key.remoteJid), key.id);
                return msg?.message || { conversation: '' };
            },
