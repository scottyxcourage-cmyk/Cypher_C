...
async function anticallCommand(sock, chatId, message, args) {
    ...
    if (sub==='on')  { save({enabled:true});  return reply(sock, chatId, '✅ Anti-call ON! All calls will be rejected.', message); }
    if (sub==='off') { save({enabled:false}); return reply(sock, chatId, '❌ Anti-call OFF.', message); }
}
