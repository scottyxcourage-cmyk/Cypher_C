...
async function antilinkCommand(sock, chatId, message, args) {
    ...
    if (sub==='on')  { d[chatId]={enabled:true};  save(d); return reply(sock, chatId, '✅ Anti-link ON!', message); }
    if (sub==='off') { d[chatId]={enabled:false}; save(d); return reply(sock, chatId, '❌ Anti-link OFF.', message); }
}
