...
async function welcomeCommand(sock, chatId, message, args) {
    ...
    if (sub==='on')  { d[chatId]={...d[chatId],enabled:true};  save(d); return reply(sock, chatId, '✅ Welcome ON!', message); }
    if (sub==='off') { d[chatId]={...d[chatId],enabled:false}; save(d); return reply(sock, chatId, '❌ Welcome OFF.', message); }
    if (sub==='set') { const msg=args.slice(1).join(' ').trim(); if(!msg) return reply(sock,chatId,'❌ Provide a message.',message); d[chatId]={...d[chatId],msg,enabled:true}; save(d); return reply(sock,chatId,'✅ Welcome message set!',message); }
}
