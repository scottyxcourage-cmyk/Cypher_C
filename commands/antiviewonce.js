...
module.exports = async (sock, chatId, message, args) => {
    ...
    if (!sub) return reply(sock, chatId, `👁️ *Anti-View Once*\nStatus: ${d.enabled?'✅ ON':'❌ OFF'}\n\n.antiviewonce on\n.antiviewonce off`, message);
    if (sub==='on')  { save({enabled:true});  return reply(sock, chatId, '✅ Anti-view-once ON! View-once media will be revealed.', message); }
    if (sub==='off') { save({enabled:false}); return reply(sock, chatId, '❌ Anti-view-once OFF.', message); }
};
